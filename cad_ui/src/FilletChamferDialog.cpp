...
namespace cad_ui {
    ...
    bool FilletChamferDialog::eventFilter(QObject* obj, QEvent* event) {
        if (event->type() == QEvent::FocusIn) {
            // Highlight the related face when the input box gains focus

            if (obj == m_chamferDistance1SpinBox) {
                emit highlightFaceRequested(0); // Corresponds to adjacentFaces[0]

            }
            else if (obj == m_chamferDistance2SpinBox) {
                emit highlightFaceRequested(1); // Corresponds to adjacentFaces[1]

            }
        }
        else if (event->type() == QEvent::FocusOut) {
            // Clear the highlight when the input box loses focus

            emit clearHighlightRequested();
        }

        // Call the base event filter to preserve default behavior
